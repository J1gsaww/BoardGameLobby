...
import {
  ...
  forcedLead, OPENING_CARD, rankIndex
} from './cards.js';
...
export function createRound({ seats, roundNo = 1, prevRanking = null, hands = null, rng = Math.random }) {
  if (!seats || seats.length < 2) throw new Error('ต้องมีที่นั่งอย่างน้อย 2 ที่');

  const dealt = hands || Object.fromEntries(
    deal(seats.length, rng).map((h, i) => [seats[i], h])
  );
  ...
  if (prevRanking && prevRanking.length) {
    ...
    state.turn = seats.find(u => state.hands[u].includes(OPENING_CARD)) || seats[0];
    state.mustInclude = OPENING_CARD;
    state.dir = 1;
  }
  return state;
}
...
export function whyNotPlay(s, uid, cards) {
  ...
  if (!readCombo(cards)) return 'notACombo';
  if (s.mustInclude && !cards.includes(s.mustInclude)) return 'mustIncludeOpening';
  if (!beats(cards, s.pile)) return 'tooWeak';
  return null;
}

export const canPlay = (s, uid, cards) => whyNotPlay(s, uid, cards) === null;
...
export function apply(state, action) {
  const s = clone(state);
  if (action.type === 'play') {
    const why = whyNotPlay(s, action.uid, action.cards);
    if (why) return { state, error: why };
    doPlay(s, action.uid, action.cards);
...
function doPlay(s, uid, cards) {
  const combo = readCombo(cards);
...
export function timeoutAction(s) {
  ...
  return s.pile
    ? { type: 'pass', uid }
    : { type: 'play', uid, cards: forcedLead(s.hands[uid]) };
}
