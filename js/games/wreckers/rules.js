...
export function actionsFor(st, uid) {
  ...
  /* ถูกบังคับให้เปิด — ทำได้อย่างเดียวคือเปิดหนึ่งในสองใบที่เขาชี้ไว้
     ไม่ใช่ตาของเราด้วยซ้ำ จึงต้องเช็กก่อนด่านตรวจตาเหมือนกัน */
  if (st.forced) return st.forced.who === uid ? ['activate'] : [];

  /* การ์ดค้างรอให้คนเปิดเลือกเป้า — ต้องอยู่เหนือด่านตรวจตาด้วย
     เพราะคนเปิดอาจเป็นคนที่ถูกบังคับ ซึ่งไม่ใช่เจ้าของตา
     ระหว่างนั้นคนอื่นทำอะไรไม่ได้เลย รวมถึงเจ้าของตาเองด้วย */
  if (st.pending) return st.pending.by === uid ? ['useCard'] : [];
