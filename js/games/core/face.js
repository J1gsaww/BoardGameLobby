...
const LABEL = { T: '10', 1: 'A' };          /* '1' คือแต้ม A ของ Yahhh */
/* ดาวเป็นดอกที่ห้าของ Yahhh — เกมอื่นไม่มีดอกนี้ การเติมจึงไม่กระทบใคร */
const GLYPH = { C: '\u2663', D: '\u2666', H: '\u2665', S: '\u2660', X: '\u2605' };
const RED = { D: 1, H: 1, X: 1 };           /* ดาวใช้สีแดงเหมือนข้าวหลามตัด */

const face = (card) => LABEL[card[0]] || card[0];
const isJoker = (card) => card[0] === 'X';
...
export function cardFace(card, w = 72) {
  ...
  if (isJoker(card)) return jokerFace(card, w, h);
...
/* โจ๊กเกอร์ — ไม่มีแต้มไม่มีดอก ใช้ดาวกับตัวอักษรแทน จะได้แยกออกทันทีตอนซ้อนกัน */
function jokerFace(card, w, h) {
  const ink = (card === 'X1' || card === 'X3') ? '#c8324a' : '#1d1b2e';
  return `<svg class="cardface" viewBox="0 0 ${w} ${h}" width="${w}" height="${h}" aria-label="Joker">
    <rect x="1" y="1" width="${w - 2}" height="${h - 2}" rx="${Math.round(w * 0.1)}"
          fill="#f6f2ea" stroke="#2a2340" stroke-width="1.5"/>
    <text x="${Math.round(w * 0.23)}" y="${Math.round(w * 0.4)}" fill="${ink}"
          font-size="${Math.round(w * 0.27)}" font-weight="700" text-anchor="middle"
          font-family="'Chakra Petch', system-ui, sans-serif">JK</text>
    <text x="${Math.round(w * 0.23)}" y="${Math.round(w * 0.72)}" fill="${ink}"
          font-size="${Math.round(w * 0.32)}" text-anchor="middle">\u2605</text>
    <text x="${Math.round(w * 0.68)}" y="${Math.round(h * 0.86)}" fill="${ink}"
          font-size="${Math.round(w * 0.34)}" text-anchor="middle" opacity=".9">\u2605</text>
  </svg>`;
}
