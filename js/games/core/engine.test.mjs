...
import { SLAVE as R } from '../slave/rules.js';
...
group('รอบแรก เริ่มที่ดอกจิก 3');
{
  ...
  const s = createRound({ seats: SEATS, hands, rules: R });
  ok('B เป็นคนเริ่ม', s.turn, 'B');
  ok('ทิศตั้งต้นตามเข็ม', s.dir, 1);
  ok('บังคับลงดอกจิก 3', s.mustInclude, '3C');
  ok('ลงใบอื่นก่อนไม่ได้', whyNotPlay(s, 'B', ['7D'], R), 'mustIncludeOpening');
  ok('คนเริ่มกองผ่านไม่ได้', whyNotPass(s, 'B'), 'mustLead');
  ok('ไม่ใช่ตาตัวเองลงไม่ได้', whyNotPlay(s, 'A', ['5H'], R), 'notYourTurn');
  ok('ไพ่ไม่ได้อยู่ในมือ ลงไม่ได้', whyNotPlay(s, 'B', ['2S'], R), 'notInHand');
...
group('กองปกติ ทุกคนผ่าน คนลงล่าสุดชนะกอง');
{
  ...
  const s = run(createRound({ seats: SEATS, hands, rules: R }), [
...
group('ผ่านแล้วออกจากกองนั้นเลย');
{
  ...
  ok('C จะลงอีกไม่ได้แม้ไพ่จะใหญ่กว่า', whyNotPlay(s, 'C', ['KD'], R), 'notYourTurn');
}
...
group('ข้อ 6 — คนหมดมือ แต่คนอื่นยังลงทับได้ ทิศไม่เปลี่ยน');
{
  ...
  let s = run(createRound({ seats: SEATS, hands, rules: R }), [
...
group('ข้อ 7 — คนชนะกองไม่มีไพ่แล้ว ทิศกลับ');
{
  ...
  const s0 = createRound({ seats: SEATS, hands, rules: R });
...
group('ล้มคิง');
{
  ...
  const s0 = createRound({ seats: SEATS, hands, rules: R, roundNo: 2, prevRanking: ['A', 'C', 'D', 'B'] });
...
group('ตาที่หมดเวลา');
{
  ...
  ok('คนเริ่มกองหมดเวลา ถูกบังคับลงเลขน้อยสุดทั้งชุด',
     timeoutAction(s, R), { type: 'play', uid: 'A', cards: ['3C', '3D'] });

  const s2 = apply(s, timeoutAction(s, R), R).state;
  ok('ลงเป็นคู่ กองนี้กลายเป็นสายคู่', s2.pile.cards.length, 2);
  ok('คนที่ไม่ได้เริ่มกองหมดเวลา ให้ผ่าน',
     timeoutAction(s2, R), { type: 'pass', uid: s2.turn });
}
...
group('จำนวนไพ่ในมือ ข้อมูลที่ทุกคนต้องเห็น');
{
  ...
  const s = apply(createRound({ seats: SEATS, hands, rules: R }), { type: 'play', uid: 'A', cards: ['3C'] }).state;
...
group('เล่นจนจบรอบด้วยไพ่ที่สับจริง');
{
  ...
  let s = createRound({ seats: ['P0', 'P1', 'P2', 'P3', 'P4'], rng, rules: R });
  let guard = 0;
  while (s.phase === PHASE.PLAY && guard++ < 4000) {
    ...
    for (const c of hand) {                       // ลองลงทีละใบจากเล็กไปใหญ่
      const r = apply(s, { type: 'play', uid, cards: [c] }, R);
      if (!r.error) { s = r.state; done = true; break; }
    }
    if (!done) {
      const r = apply(s, { type: 'pass', uid }, R);
