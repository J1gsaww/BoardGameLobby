...
export function beats(cards, pile, v = BASE, rev = false) {
  ...
  // โจ๊กเกอร์เดี่ยวชนะ **ไพ่เดี่ยว** ทุกใบ แต่มีใบเดียวที่ล้มมันได้
  ...
  //
  // **ข้อยกเว้นนี้ใช้กับไพ่เดี่ยวด้วยกันเท่านั้น**
  // ตองยังทับโจ๊กเกอร์เดี่ยวได้ตามปกติ เพราะในสายคี่ชุดใหญ่ชนะชุดเล็กเสมอ
  // ของเดิมเขียนกว้างไป เลยบล็อกตองไปด้วยทั้งที่ไม่เกี่ยวกับกฎใบล้มเลย
  if (v.jokers) {
    if (top.soloJoker) {
      if (cards.length === 1) return cards[0] === jokerKiller(rev);
